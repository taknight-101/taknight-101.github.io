---
layout: post
title: "Utility Javascript Functions to Reuse in ServiceNow Development"
permalink: "u1"
...
#### Provided are samples and their usages for different use cases:
  ...
  for (var i = 0; i < numRecords; i++) {
    ...
    for (var field in fieldsAndValues) {
      if (fieldsAndValues.hasOwnProperty(field)) {
        ...
        if (
          JSON.stringify(gr[field].getED().getInternalType()) === '"reference"'
        ) {
