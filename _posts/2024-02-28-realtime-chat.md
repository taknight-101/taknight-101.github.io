...
Hello again <br>
**In this blog, We are going to build a real time chat application in the servicenow platform from start to finish**

I had some goals in mind that i wanted to tackle regarding why to implement such application in such platform, so i will list them first and foremost
...
which you configure to listen to updates to any tables you wish and be notified about the event that just happened and even have access to the event payload of what has happened, and that's what we want to achieve real time communication inside servicenow as we please, so now, let's get down to the implementation
...
We start by navigating to `app engine studio` for faster application bootstrapping, went ahead with choosing the name `chat-app` and default 2 roles for admin and user to have access to the application and its features.
...
This is the script that fetches the data from the servicenow tables, and as seen above this resolver is the `Get User` resolver, which handles the `User type` defined in schema as
...
And that's all. We've seen an approach to tackle achieving backend communication patterns used in lots of full stack application in a platform engineering context, and saw how the platform could provide hooks to ease the implementation throughout the way.

Thank you all for reaching this far, and hope you enjoyed this blog as well as i hope to see you soon in another blog
